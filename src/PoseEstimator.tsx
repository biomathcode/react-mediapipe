			...
			try {
				if (runningMode === "VIDEO") {
					...
					return await new Promise((resolve) => {
						pose.detectForVideo(input as HTMLVideoElement, now, (res: any) =>
							resolve(res),
						);
					});
				} else {
						...
						if (result.landmarks && drawSkeleton) {
							for (const landmark of result.landmarks) {
								...
								drawingUtils.drawLandmarks(landmark, {
									radius: (data: any) =>
										poseRef.current.DrawingUtils.lerp(
											data.from?.z,
											-0.15,
											0.1,
											5,
...
export default function PoseEstimator(props: PoseEstimatorOptions) {
	...
	useEffect(() => {
		...
		// keep deps minimal
	}, [streaming, ready, onResults, startLoop, stopLoop]);
	...
	return (
		<div className={`mediapipe-pose-estimator ${className} p-2`}>
			<div className="flex-row">
				<button className="btn" onClick={toggleCamera} aria-pressed={streaming}>
			...
			<div className="grid">
				...
				>
					...
					{/* DIV OVERLAY (TOP) */}
					{!streaming && (
						<div className="overlay">Camera is off — press Start Camera</div>
					)}
				</div>
				...
				<div className="p-2">
					...
					<ul className="text-xs mt-2">
						<li>
							Model:{" "}
							{modelUrl
								? new URL(modelUrl).pathname.split("/").pop()
								: "default"}
						</li>
